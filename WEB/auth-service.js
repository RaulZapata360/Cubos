...
class AuthService {
    ...
    async updateProfile(updates) {
        try {
            ...
            // También intentar actualizar la tabla usuarios si es necesario
            await supabaseClient
                .from('usuarios')
                .update({
                    nombre_completo: updates.nombre,
                    // Si la tabla tiene estos campos, se actualizarán; si no, fallará silenciosamente o se ignorará según RLS
                })
                .eq('id', data.user.id);

            this.currentUser = data.user;
            sessionStorage.setItem('userName', updates.nombre);
