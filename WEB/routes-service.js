...
class RoutesService {
    ...
    async obtenerDatosRuta(origenId, destinoId) {
        try {
            ...
            const isObraRoute = (this.currentObraId && (origenId === this.currentObraId || destinoId === this.currentObraId));

            let cachedData = null;
            if (!isObraRoute) {
                cachedData = await this.getCachedRouteData(origenId, destinoId);
            }
            ...
            // Only try if not an obra route (to avoid 406 errors)
            const isObraRoute = (this.currentObraId && (origenId === this.currentObraId || destinoId === this.currentObraId));
            if (!isObraRoute) {
    ...
    async getAddresses(origenId, destinoId) {
        ...
        const getLocation = async (id) => {
            // CHECK IF IT IS THE CURRENT OBRA (Optimization to avoid 406 errors)
            // If we know this ID is the current obra, query 'obras' table directly
            if (this.currentObraId && id === this.currentObraId) {
